...
def create_image_lists(testing_percentage, validation_percentage):
    ...
    result = {}
    # 获取目录下所有子目录
    sub_dirs = [x[0] for x in os.walk(gDir.INPUT_DATA)]
    ...
    for sub_dir in sub_dirs:  # 遍历目录数组，每次处理一种
        if is_root_dir:
            ...
            # 获取当前目录下所有的有效图片文件
        #extensions = ['png','PNG','jpg', 'jepg', 'JPG', 'JPEG']
        file_list = []
        dir_name = os.path.basename(sub_dir)  # 返回路径名路径的基本名称，如：daisy|dandelion|roses|sunflowers|tulips
        for extension in gDir.IMG_EXTENSIONS:
            file_glob = os.path.join(gDir.INPUT_DATA, dir_name, '*.' + extension)  # 将多个路径组合后返回
...
def get_image_path(image_lists, image_dir, label_name, index, category):
    ...
    #print("label_name:",label_name,"  category:",category)
    try:
...
def get_bottleneck_path(image_lists, label_name, index, category):
    return get_image_path(image_lists, gDir.CACHE_DIR, label_name, index, category) + '.txt'  # CACHE_DIR 特征向量的根地址
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, category, jpeg_data_tensor, bottleneck_tensor):
    ...
    label_lists = image_lists[label_name]
    sub_dir = label_lists['dir']
    sub_dir_path = os.path.join(gDir.CACHE_DIR, sub_dir)  # 到类别的文件夹
    ...
    if not os.path.exists(bottleneck_path):  # 如果不存在
        # 获取图片原始路径
        image_path = get_image_path(image_lists, gDir.INPUT_DATA, label_name, index, category)
        # 获取图片内容
